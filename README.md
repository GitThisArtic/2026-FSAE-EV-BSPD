...
### Schematic Quick Notes
Currently the schematic and the PCB have mismatching idenifiers for each component. Will be fixed in the near future
...
## TRACTIVE SYSTEM LOGIC [Values still need to be measured]
The tractive system current is determined based on the current input voltage of the throttle pedal. With a current of a tractive system current of _____ around at ____kW supplied to the motors under the 403.2V tractive system voltage. [Values still need to be measured]

## BSPD LOGIC
Both the tractive system logic and the braking logic are passed through CD4071BPWR OR gate to check whether the input signal is too low or too high from each logic block respectively. The logic of the two blocks are then checked with an SN74AHC08DR AND gate to determine if the vehicle is simultaneously undergoing acceleration and hard braking.
